...
export function DiscoHint() {
  const reduce = useReducedMotion();
  const [show, setShow] = useState(false);

  const getCount = () => { try { return parseInt(sessionStorage.getItem(COUNT_KEY) ?? "0", 10); } catch { return MAX; } };
  const isClicked = () => { try { return sessionStorage.getItem(CLICKED_KEY) === "1"; } catch { return true; } };
  const bumpCount = useCallback(() => { try { sessionStorage.setItem(COUNT_KEY, String(getCount() + 1)); } catch {} }, []);

  useEffect(() => {
    if (typeof window === "undefined" || isClicked()) return;
    let mounted = true;
    const timers: ReturnType<typeof setTimeout>[] = [];

    const popOnce = (ms: number) => {
      if (!mounted || isClicked() || getCount() >= MAX) return;
      setShow(true);
      bumpCount();
      timers.push(setTimeout(() => { if (mounted) setShow(false); }, ms));
    };

    timers.push(setTimeout(() => popOnce(5000), 2000));

    const scheduleNext = () => {
      if (!mounted || isClicked() || getCount() >= MAX) return;
      const delay = 60000 + Math.random() * 30000;
      timers.push(setTimeout(() => { popOnce(4000); scheduleNext(); }, delay));
    };
    timers.push(setTimeout(scheduleNext, 8000));
      ...
      {show && (
        <motion.button
          ...
          transition={
            reduce
              ? { duration: 0.2 }
              : { y: { repeat: Infinity, duration: 1.4 }, default: { type: "spring", stiffness: 220, damping: 14 } }
          }
