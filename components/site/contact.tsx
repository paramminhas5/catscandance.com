...
export function Contact() {
  ...
  const hpY = useTransform(scrollYProgress, [0, 1], reduce ? [0, 0] : [-80, 80]);
  const hpRot = useTransform(scrollYProgress, [0, 1], reduce ? [0, 0] : [-10, 20]);

  const [form, setForm] = useState({ name: "", email: "", message: "", reason: REASONS[0] as Reason });
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      setSent(true);
      setForm({ name: "", email: "", message: "", reason: REASONS[0] as Reason });
    });
  }
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 md:px-8">
        ...
        >
          ...
          <p className="text-ink/80 text-base sm:text-lg mt-4 max-w-xl">
            Brand collabs, venue partnerships, press, or just to send us a cat photo. We read
            everything within 24h.
          </p>
        </motion.div>
        ...
        >
          {sent ? (
            <div className="py-10 text-center">
              ...
              <p className="text-ink/70 font-medium mb-6">
